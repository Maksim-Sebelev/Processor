...
static AssemblerErr HandlePush(AsmData* AsmDataInfo)
{
    ...
    {
        ASSEMBLER_ASSERT(PushTypeCtor(&Push, 1, 0, 0));
        SetElem = PushElem;
    }

    else if (IsRegister(buffer, BufferLen))
    {
        ASSEMBLER_ASSERT(PushTypeCtor(&Push, 0, 1, 0));
    ...
    else if (IsMemory(buffer, BufferLen))
    {
        ...
        {
            COLOR_PRINT(GREEN, "b\n");
            ASSEMBLER_ASSERT(PushTypeCtor(&Push, 0, 0, 1));
            SetElem = PushElemMemIndex;
        }

        else if (IsRegister(buffer, BufferLen))
        {
            ASSEMBLER_ASSERT(PushTypeCtor(&Push, 0, 1, 1));
            SetElem = GetRegisterPointer(buffer);
        }
    }
...
static AssemblerErr HandleAdd(AsmData* AsmDataInfo)
{
    ...
    return NullArgCmdPattern(AsmDataInfo, add);
}
...
static AssemblerErr HandleSub(AsmData* AsmDataInfo)
{
    ...
    return NullArgCmdPattern(AsmDataInfo, sub);
}
...
static AssemblerErr HandleMul(AsmData* AsmDataInfo)
{
    ...
    return NullArgCmdPattern(AsmDataInfo, mul);
}
...
static AssemblerErr HandleDiv(AsmData* AsmDataInfo)
{
    ...
    return NullArgCmdPattern(AsmDataInfo, dive);
}
...
static AssemblerErr HandleOut(AsmData* AsmDataInfo)
{
    ...
    return NullArgCmdPattern(AsmDataInfo, out);
}
...
// static int GetLabelCodePlace(AsmData* AsmDataInfo, int Labels_i)
// {
//     assert(AsmDataInfo);

//     return AsmDataInfo->Labels.Labels[Labels_i].CodePlace;
// }
...
static void PrintError(AssemblerErr* err)
{
    ...
    switch (err->err)
    {
        ...
        case AssemblerErrorType::INVALID_INPUT_AFTER_PUSH:
            COLOR_PRINT(RED, "Error: invalis input after push.\n");
            break;
