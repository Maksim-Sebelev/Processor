...
## Прыжки:
  Все следующие команды, кроме `jmp` имеют в качестве аргумента метку, указывающую место в коде, куда должен перейти процессор.
...
`ret` - делает прыжок по массиву кода на индекс, численно равный элементу на верхушке стека и удаляет верхушку стека.
...
В папке `/examples/` лежат 4 папки с примерами простейших программ. 
